<template>
  <div>
        ...
        <el-form-item label="课程Id">
          <el-input v-model="searchInfo.courseId" placeholder="搜索条件" />
           ...
           <el-form-item label="状态" prop="state">
            <el-select v-model="searchInfo.state" clearable placeholder="请选择" @clear="()=>{searchInfo.state=undefined}">
              <el-option v-for="(item,key) in statusOptions" :key="key" :label="item.label" :value="item.value" />
            </el-select>
            </el-form-item>
           <el-form-item label="是否审核" prop="audit">
            <el-select v-model="searchInfo.audit" clearable placeholder="请选择" @clear="()=>{searchInfo.audit=undefined}">
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="考试名称" prop="name" width="120" />
        <el-table-column align="left" label="教学班id" prop="teachClassId" width="120" />
        <el-table-column align="left" label="考试时长" prop="time" width="120" />
        <el-table-column align="left" label="考试时间" prop="startTime" width="120" />
        ...
        <el-table-column align="left" label="考试模板Id" prop="templateId" width="120" />
        <el-table-column align="left" label="状态" prop="state" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.state,statusOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="是否审核" prop="audit" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.audit,auditOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="考试类型" prop="type" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.type,ExamTypeOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="通过分数" prop="passScore" width="120" />
        <el-table-column align="left" label="权重" prop="weight" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateExamPlanFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="是否审核:"  prop="audit" >
          <el-select v-model="formData.audit" placeholder="请选择" style="width:100%" :clearable="true" >
            <el-option v-for="(item,key) in auditOptions" :key="key" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="考试类型:"  prop="type" >
        ...
        <el-form-item label="通过分数:"  prop="passScore" >
          <el-input-number v-model="formData.passScore"  style="width:100%" :precision="2" :clearable="true"  />
        </el-form-item>
...
const getTableData = async() => {
  const table = await getExamPlanList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteExamPlanByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateExamPlanFunc = async(row) => {
    const res = await findExamPlan({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteExamPlanFunc = async (row) => {
    const res = await deleteExamPlan({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
