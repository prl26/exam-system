...
func (draftPaperService *DraftPaperService) ConvertDraftCheck(info request.ConvertDraft) (IsOk bool, err error) {
	...
	err = global.GVA_DB.Raw("SELECT SUM(score) FROM `exam_draft_paper_merge` where draft_paper_id = ? and deleted_at is null", info.DraftPaperId).Scan(&totalScore).Error
	if err != nil {
		return false, err
	}
	if totalScore == 100 {
